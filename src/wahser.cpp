...
void sampling(long double driver_prop, long double pre_prop, long double ond_prop) {
	...
	// for (int i = 0; i < region_cnt[0][0].size(); i ++) {
	// 	tmp[region_cnt[0][0][i] / 60] ++;
	// }
	// int tot = 0;
	// for (int i = 0; i < 60; i ++) {
	// 	printf("%d\t%d\n", i, tmp[i]);
    ...
    freopen("pre_requests.txt", "w", stdout);
    printf("%d\n", pre_num);
    ...
    freopen("ond_requests.txt", "w", stdout);
    printf("%d\n", ond_num);
